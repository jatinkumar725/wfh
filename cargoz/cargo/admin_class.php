...
Class Action {
	...
	function save_branch(){
		...
		if(empty($id)){
			...
			$i = 0;
			while($i == 0){
				$bcode = "BRA-".substr(str_shuffle($chars), 0, 15);
	...
	function save_parcel(){
		...
		foreach($price as $k => $v){
			...
			if(!isset($type)){
				$getType = $this->db->query("SELECT * FROM `users` WHERE email = '$email' order by id DESC limit 0,1");
				$result = $getType->fetch_array();
				$type = $result['type'];
				$data .= ", type='$type' ";
			}
	...
	function save_franchise()
	{
		...
		if(empty($id)){
			...
			$i = 0;
			while($i == 0){
				$bcode = "FRAN-".substr(str_shuffle($chars), 0, 15);
				$chk = $this->db->query("SELECT * FROM branches where branch_code = '$bcode'")->num_rows;
				if($chk <= 0){
					$i = 1;
				}
	...
	function delete_franchise()
	{
		...
		if ($delete) {
			extract($_POST);
			$delete = $this->db->query("DELETE FROM users where branch_id = ".$id);
			if($delete)
				return 1;
		}
	}
